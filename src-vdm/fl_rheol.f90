...
dA = -real(cord(1,iinj-1,2)+cord(1,iinj,2)+cord(1,iinj+1,2))*1.e-3/3.
pi = 3.14159
tau_m = 3.1536e12   ! 0.1Ma
rate_inject_0 = rate_inject + 1.6e-10*sin((2.*pi*time)/tau_m)
rate_inject_1 = rate_inject_0 + dA*0.2*1.6e-9
if (rate_inject_1.ge.1.6e-9) then
rate_injcet_1 = 1.6e-9
end if
...
!         write (*,*) nelem_inject
         ...
         do jinj = 1,nelem_inject
            ...
            dxinj=dxinj+cord(jinj,iinj+1,1)-cord(jinj,iinj-1,1)
         enddo
...
!            if (ny_inject .gt.0 .and.i.le.iinj) visn(j,i) = v_min
        ...
        do k = 1,4
            ...
            if(ny_inject.gt.0.and.j.le.nelem_inject) then
                ...
                if(i.ge.iinj-1.and. i.le.iinj) then
...
!
            if (aps (j,i).lt.0.) aps (j,i) = 0.
            if (ny_inject.gt.0.and.i.le.iinj .and.i.ge.iinj-1) aps (j,i) = 0.00000  ! limited strain in Dike,Liu
